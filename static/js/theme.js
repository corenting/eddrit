...
function setTheme(theme) {
	...
	// Else, set the theme corresponding to matching dark mode media query
		...
		document
			...
			.setAttribute(
				"data-theme",
				DARK_MODE_MEDIA_QUERY.matches ? THEME_DARK : THEME_LIGHT,
			);
	}
}
...
function getActiveTheme() {
	...
	const userTheme = window.localStorage.getItem(THEME_LOCAL_STORAGE_KEY);
	if (userTheme) {
		return userTheme;
	}
...
function setupSettingsPage() {
	const selectElement = document.getElementById("settings-theme-select");
